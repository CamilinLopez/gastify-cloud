...
const PrintCheckbox: React.FC<PrintCheckboxProps> = ({ roles, formValues, setRoles }) => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const res = await axiosInstance.put(`/roles/asignar-permiso/${rolSeleccionado.id}`,{permisos:selectedPermissions})
      

      await setRoles((prevRoles) =>
        prevRoles.map((rol) =>
          rol.id === res.data.data.id ? res.data.data : rol
        )
      );
        ...
        {permisos.length > 0 ? (
          permisos.map((permiso) => (
            <div key={permiso.id} className="flex gap-x-2 my-3">
              ...
              />
              <label htmlFor={`permiso-${permiso.id}`} className="text-16px">
                {permiso.nombre}
              </label>
            </div>
          ))
